...
let sliderCounter = 0;
let sliderWidth; // счетчик - специальная переменная - на нее буду опираться в расчетах для свойства transform: translateX()
...
function resizeSlider() {
  sliderWidth = document.querySelector(".carousel__slider").offsetWidth;

  sliderLine.style.width = sliderWidth * sliderImgs.length + "px";

  sliderImgs.forEach(img => img.style.width = sliderWidth + "px");
}
...
function moveSlide(sliderCount) { // функция расчета перемещения слайдов
  ...
  // sliderWidth = slider.offsetWidth; // получение ширины блока слайдера в переменную (550px) - для определения насколько нужно смещать по оси X слайды, чтобы они меняли друг друга
